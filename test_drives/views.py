...
def edit_test_drive(request, test_drive_id):
    test_drive = get_object_or_404(
        TestDrive, id=test_drive_id, user=request.user
    )
    if request.method == "POST":
    ...
    return render(
        request,
        "test_drives/edit_test_drive.html",
        {"form": form, "test_drive": test_drive},
    )
...
def delete_test_drive(request, test_drive_id):
    ...
    return render(
        request,
        "test_drives/delete_test_drive.html",
